...
namespace EAMIS.Core.BusinessLogic.Masterfiles
{
    public class EamisUsersRepository : IEamisUsersRepository
    {
        ...
        public AisPersonnelDTO getEmployeeName(AisPersonnelDTO item)
        {
            if (item == null) return new AisPersonnelDTO();
            var personnel = _aisctx.Personnel.AsNoTracking().Where(x => x.AgencyEmployeeNumber == item.AgencyEmployeeNumber);

            return new AisPersonnelDTO
            {
                FirstName = personnel.FirstOrDefault().FirstName,
                MiddleName = personnel.FirstOrDefault().MiddleName,
                LastName = personnel.FirstOrDefault().LastName
