...
namespace EAMIS.Core.LogicRepository.Classification
{
    public class EamisSubClassificationRepository : IEamisSubClassificationRepository
    {
        ...
        public async Task<List<EAMISSUBCLASSIFICATION>> ListSubClassifications(string searchValue)
        {
            var result = _ctx.EAMIS_SUB_CLASSIFICATION.AsNoTracking().Where(x => x.NAME_SUBCLASSIFICATION == searchValue).ToList();
            return result;
        }
