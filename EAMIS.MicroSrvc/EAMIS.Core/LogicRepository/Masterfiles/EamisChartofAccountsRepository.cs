...
namespace EAMIS.Core.LogicRepository.Masterfiles
{
    public class EamisChartofAccountsRepository : IEamisChartofAccountsRepository
    {
        ...
        public Task<bool> ValidateExistingCode(string accountcode)
        {
            return _ctx.EAMIS_CHART_OF_ACCOUNTS.AsNoTracking().AnyAsync(x => x.ACCOUNT_CODE == accountcode);
        }

        public Task<bool> UpdateValidateExistingCode(string accountcode, int id)
        {
            return _ctx.EAMIS_CHART_OF_ACCOUNTS.AsNoTracking().AnyAsync(x => x.ACCOUNT_CODE == accountcode && x.ID == id);
        }
    }
}
