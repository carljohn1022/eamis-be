...
namespace EAMIS.Core.LogicRepository.Masterfiles
{
    public class EamisWarehouseRepository : IEamisWarehouseRepository
    {
        ...
        public Task<bool> ValidateExistingCode(string warehouseDescription)
        {
            return _ctx.EAMIS_WAREHOUSE.AsNoTracking().AnyAsync(x => x.WAREHOUSE_DESCRIPTION == warehouseDescription);
        }
        public Task<bool> UpdateValidateExistingCode(string warehouseDescription, int id)
        {
            return _ctx.EAMIS_WAREHOUSE.AsNoTracking().AnyAsync(x => x.WAREHOUSE_DESCRIPTION == warehouseDescription && x.ID == id);
        }
