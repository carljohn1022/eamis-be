...
namespace EAMIS.WebApi.Controllers.Transaction
{
    ...
    public class EamisServiceLogController : ControllerBase
    {
        ...
        {
            var nextId = await _eamisServiceLogRepository.GetNextSequenceNumber(branchID);
            return nextId;
        }
        ...
        //}
        [HttpGet("editbyid")]
        public async Task<ActionResult<EamisPropertyTransactionDTO>> getServiceItemById(int itemID)
        {
            return Ok(await _eamisServiceLogRepository.getServiceItemById(itemID));
        }
    }
}
