...
namespace EAMIS.WebApi.Controllers.Transaction
{
    ...
    public class EamisPropertyTransferDetailsController : ControllerBase
    {
        ...
        //}
        [HttpGet("listitemsfortransfer")]
        public async Task<ActionResult<EamisPropertyTransferDetailsDTO>> List([FromQuery] EamisPropertyTransferDetailsDTO filter, [FromQuery] PageConfig config, string tranType)
        {
            if (filter == null)
                filter = new EamisPropertyTransferDetailsDTO();
            return Ok(await _eamisPropertyTransferDetailsRepository.ListItemsForTranser(filter, tranType, config));
        }
