...
namespace EAMIS.WebApi.Controllers.Masterfiles
{
    ...
    public class EamisUsersController : ControllerBase
    {
       ...
       [HttpGet("GetUsername")]
        ...
        [HttpGet("GetEmployee")]
        public ActionResult<AisPersonnelDTO> GetEmployee([FromQuery] AisPersonnelDTO item)
        {
            if (item == null)
                item = new AisPersonnelDTO();
            return Ok(_applicationUsers.getEmployeeName(item));
        }
